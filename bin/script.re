...
let rec add_friends' =
  ...
  | [] => ()
  | [(f1, f2), ...t] => {
      ignore(add_friends(f1, f2));
      add_friends'(t);
    };

let rec add_group_info' =
  fun
  | [] => ()
  | [(gn, hid), ...t] => {
      ignore(add_group_info(gn, hid));
      add_group_info'(t);
    };
  ...
  | [(gid, mid), ...t] => {
      ignore(
        add_group_invites(
          gid,
          mid,
          snd(List.nth(group_info_list, gid - 1)),
        ),
      );
      ignore(join_group(gid, mid));
      add_groups'(t);
    };

let rec add_restrictions' =
  fun
  | [] => ()
  | [(uid, rid), ...t] => {
      ignore(add_restrictions(uid, rid));
      add_restrictions'(t);
    };

let rec add_rest_idx' =
  fun
  | [] => ()
  | [(admin, rest_name), ...t] => {
      ignore(add_restrictions_index(admin, rest_name));
      add_rest_idx'(t);
    };

let rec add_pref_idx' =
  fun
  | [] => ()
  | [(admin, preference), ...t] => {
      ignore(add_preferences_index(admin, preference));
      add_pref_idx'(t);
    };

let rec add_cuisine_idx' =
  fun
  | [] => ()
  | [(admin, id, cuisine), ...t] => {
      ignore(add_cuisine(admin, id, cuisine));
      add_cuisine_idx'(t);
    };
